    ...
    <main className='relative flex flex-col h-screen p-1 overflow-hidden dark:bg-light-grey'>
        ...
        {messages?.length ? (
          ...
          <div className='flex my-2'>
            <div className='w-screen overflow-hidden'>
              <ul className='grid grid-cols-2 gap-3 mx-5 rounded-lg'>
                {template.map((item, index) => (
                  <li
                    onClick={() => setFormValue(item.prompt)}
                    key={index}
                    className='p-2 border rounded-lg border-slate-300 hover:border-slate-500'>
      ...
      <form
        ...
        <select
          ...
          className='w-full sm:w-40 mb-2 sm:mb-0 select select-bordered join-item'>
          {options.map((option, index) => (
            <option key={index} value={option}>{option}</option>
          ))}
        </select>
        
        <div className='flex items-stretch justify-between w-full border-t dark:border-gray-100 border-gray-900'>
          
          <textarea
            ref={inputRef}
            className='w-full p-2 grow input input-bordered join-item max-h-[20rem] min-h-[3rem] border rounded shadow-sm'
            value={formValue}
          ...
          <button 
            type='submit' 
            className={`join-item btn p-2 transition-all ${thinking ? 'bg-gray-400' : 'bg-blue-500 hover:bg-blue-600'} ${!formValue && 'cursor-not-allowed'}`}
