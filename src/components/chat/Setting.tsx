    ...
    <div className={`modal ${modalOpen ? 'modal-open' : ''}`}>
      <div className='modal-box'>
        <form onSubmit={saveKey} className='p-5 space-y-4'>
          ...
          <p>Input your personal OpenAI API-key below. Your keys are securely saved in your own browser.</p>
          ...
          <div className='flex gap-4'>
            <button type='submit' className='flex-1 p-2 bg-blue-600 text-white rounded hover:bg-blue-500' disabled={loading}>
              {loading ? 'Checking...' : 'Save'}
            </button>
            {apiKey && input && (
              <button type='button' onClick={loading ? undefined : removeApiKey} className='flex-1 p-2 bg-red-600 text-white rounded hover:bg-red-500' disabled={loading}>
                Remove
              </button>
            )}
          </div>
